...
{product} exposes the following metrics in Prometheus format. You can monitor your clusters with these metrics.
...
The {product} metrics are exposed under `/metrics`` at port `8000` along with {pulsar-short} metrics. Use a different port by configuring the `stats_server_port` system property. Depending on where the {product} service is deployed, each metric will be prefixed with either "pulsar_proxy", "pulsar", or "server" (for standalone).
