...
For Kubernetes deployments, use xref:kaap-operator:ROOT:index.adoc[{kaap}].
====
+
...
+
. Install the xref:installation:attachment$values.yaml[{company} Luna Helm chart] using a minimal values file.
The following command creates a Helm release named `my-pulsar-cluster` using the Helm chart within a Kubernetes namespace named `datastax-pulsar`.
