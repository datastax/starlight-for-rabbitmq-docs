...
The following configuration settings are used by {product}. Depending on how you are running {product}, the configuration file is read in a different way:

[cols=2*]
|===
|*How are you running {product}*?
...
|If set, the RabbitMQ service will use these parameters to authenticate on {pulsar-short}'s brokers. If not set, the brokerClientAuthenticationParameters setting will be used. This setting allows different credentials for the {pulsar-short} proxy and the RabbitMQ service.
