...
|`amqpHeartbeatDelay`
|`0`
|The duration, in seconds, for the Broker and client to exchange heartbeat messages when using AMQP.
Clients can negotiate a different heartbeat frequency or disable it altogether.
...
|`tokenAuthClaim`
|Specify the token claim that is used as the authentication `principal` or `role`.
If not set, the `subject` field is used.
...
|If set, the RabbitMQ service uses these parameters to authenticate on {pulsar-short}'s brokers.
